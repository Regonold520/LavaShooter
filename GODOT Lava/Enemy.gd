...
func _on_death():
	...
	if health == 0:
		...
		if death_count == 1:
			...
			var roomholder = get_parent().find_child("RoomHolder").get_child(0)
			roomholder.completed_enemies += 1
			
			print(roomholder.enemy_ammount , "  AND  " , roomholder.completed_enemies)
